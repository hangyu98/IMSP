...
import numpy as np
import os as os
from model.graph import hetero_data as hetero_data
...
def build_one_homo_graph(G, file_name, protein_name, layer_name, belong_relation_dict, node_index):
    """
    build one homo graph and merge into G
...
def init_homo_graph(G, index_name_map, similarity_matrix, protein_name, layer_name, belong_relation_dict, node_index):
    """
    initiate the DiGraph graph using networkx, put nodes and edges in the homo graph
    :param node_index:
    :param belong_relation_dict: ...
    :param G: graph G
...
def build_original_hetero_edges(G, hetero_edges, dict_of_belong_relations, dict_of_nodes_groups, list_of_virus):
    ...
    for info_dict in hetero_data.hetero_data:
        hetero_edges = hetero_edges + (add_hetero_edges(G=G,
                                                        dict_of_nodes_groups=dict_of_nodes_groups,
...
def build_home_graph(G, dict_of_nodes_groups, dict_of_edges_groups, file_dir, belong_relation_dict):
    """
    build homogeneous graph into the graph G
    :param belong_relation_dict: ...
    :param G: Graph G
...
def add_hetero_edges(G, dict_of_nodes_groups, layer_1, type_1, host_list_1, layer_2, type_2, host_list_2, relation,
    ...
    :param data: graph associated with the edge
    :param G: graph G
    ...
    for first_type in type_1:
        ...
        for nd_1 in dict_of_nodes_groups[layer_1][first_type]:
            # get the node in the graph at key 'nd_1', which returns dict of all attributes of the node
            # then, match the host
