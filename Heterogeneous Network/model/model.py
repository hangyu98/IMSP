...
class Classifier:
    ...
    def save_pred(self, X_list, index2pair_dict, full_G, all_selected_indices, prediction, prediction_prob, edge_type):
        ...
        with open(os.path.abspath(
                '../data/prediction/result/prediction_' + edge_type + '_mat_' + self.prediction_model + '.csv'),
                'w') as file:
            for i in range(0, len(X_list)):
    ...
    def fdr(self, edge_types):
        ...
        for edge_type in edge_types:
            ...
            corrected_mat = genfromtxt(
                os.path.abspath(
                    '../data/prediction/result/prediction_' + edge_type + '_adjusted_' + self.prediction_model + '.csv'),
                delimiter=',')
            ...
            with open(os.path.abspath(
                    ...
                    'r') as file:
                with open(os.path.abspath(
                        '../data/prediction/result/prediction_' + edge_type + '_' + self.prediction_model + '.csv'),
                        'w') as new_file:
